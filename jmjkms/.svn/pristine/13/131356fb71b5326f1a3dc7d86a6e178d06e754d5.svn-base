...
<body>
  ...
  <div class="main-content clearfix">
    ...
    <!-- 主容器 start -->
    <div class="container">
       <h3 class="current-title">会诊记录</h3>
      
      <!-- 删除此行注释，在此添加对应的div -->
       <div class="search-content">
       <input name="yemianname" value="06203" type="hidden"/>
       <form id="entity" action="consultationRecordAction!searchConsultationRecord.action" method="post">
    ...
    <%--  <div class="search-item">
     ...
     <div class="btn-content">
       ...
       <a class="btn-xinzeng btn" href="consultationRecordAction!findAndAddConsultationRecord.action" class="btn">添加</a>
     </div>
 	</form>
      ...
      <form action="consultationRecordAction!fuction.action" method="post">
       <table class="table table-bordered">
         <thead>
           ...
           <tbody>
            <%-- <s:action name="consultationRecordAction!showConsultationRecord.action" namespace="/" var="home1"></s:action> --%>
            <s:iterator value="vconsultationRecordList" status="n">
		       <tr>
		       <input type="hidden" name="choice" value="${consultationRecordId}">
		        <%--  <td><input class="zyq" type="checkbox" name="allConsultationRecordId" id="ck_id" value="${consultationRecordId}"></td> --%>
		        
		         <td><c:out value="${name}"></c:out></td>
		          <td>
				<c:if test="${sex==false}">男</c:if> <c:if test="${sex==true}">女</c:if>
				</td>
		         <td><c:out value="${age}"></c:out></td>
		          <td><c:out value="${fileNum}"></c:out></td>
		         <td><c:out value="${conRecDate}"></c:out></td>
		         <td colspan="3"><a  class="btn-xinzeng" href="consultationRecordAction!turnToAddPersonConsultationRecord.action?healthFileId=${healthFileId}">新增</a>
		         <a class="btn-xianshi"  href="consultationRecordAction!turnToDetailConsultationRecord.action?healthFileId=${healthFileId}&consultationId=${consultationRecordId}">详情</a>
		         <a onclick="return confirm('是否删除？');" class="btn-shanchu" href="consultationRecordAction!deleteConsultationRecord.action?consultationId=${consultationRecordId}">删除</a>
		         <a class="btn-xiugai" href="consultationRecordAction!turnToUpdateConsultationRecord.action?consultationId=${consultationRecordId}&healthFileId=${healthFileId}">修改</a></td>
