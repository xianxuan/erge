...
<body>
  ...
  <div class="main-content clearfix">
    ...
    <!-- 主容器 start -->
    <div class="container">
      <h3 class="current-title">知识讲座</h3>
      
      <!-- 删除此行注释，在此添加对应的div -->
		<div class="search-content">
		<input name="yemianname"  value="05203" type="hidden"/>
			<form class="search-form" action="knowledgeLectureAction!getKnowledgeLectureBySearch.action" method="post">
	        	     <label>讲座名称：</label>

                     <input class="temp" type="text" class="w60" type="text"  name="searchText" value="${searchText}">

	            	<label>状态选择：</label>
	            	<select name="knowledgeLectureStatus" id="select">
	            	    <option value="">请选择</option>
	              		<option value="0" <c:if test="${knowledgeLectureStatus eq '0'}">selected</c:if>>暂缓处理</option>
	              		<option value="1" <c:if test="${knowledgeLectureStatus eq '1'}">selected</c:if>>待处理</option>
	                    <option value="2" <c:if test="${knowledgeLectureStatus eq '2'}">selected</c:if>>不处理</option>
	              		<option value="3" <c:if test="${knowledgeLectureStatus eq '3'}">selected</c:if>>已处理</option>
	            	</select>
	            <label>计划时间：</label>
	            ...
	            <a class="btn btn-xinzeng" href="knowledgeLectureAction!addKnowledgeLecture.action" >新增</a>
	        </form>
		</div>
		<!-- 导出 -->
		<form action="knowledgeLectureAction!test.action" method="post">
		<div class="table-content">
			<table class="table table-bordered">
	        	<thead>
	         	...
	         	<tbody>
	         	<s:iterator value="knowlectalList" var="kg">
	         	<!-- 导出 -->
	         	<input type="hidden" name="ids" value="${kg.knowledge.knowledgeLectureId }" />
					<tr>
					<c:if test="${session.comHospital.isHead==0 }"> <!--  系统管理员-->
					    <td>${kg.communitygroup}</td>
					    <td>${kg.communityHospital}</td>
					</c:if>
				    <c:if test="${session.communityHospitalGroup !=null}">   <!-- 集团 -->
					    <td>${kg.communityHospital}</td>
					</c:if>
	             		<td><span class="dspan">${kg.knowledge.lectureName}</span></td>
	             		<td><span class="dspan">${kg.knowledge.forPerson}</span></td>
	             		<td><span class="dspan">${kg.knowledge.participatePeople}</span></td>
	             		<td><fmt:formatDate value='${kg.knowledge.planTime}' pattern='yyyy-MM-dd'/></td>
	             		
                         <c:if test="${kg.knowledge.status==0}">
                         <td><c:out value="暂缓处理"></c:out></td>
             			 </c:if>
			             <c:if test="${kg.knowledge.status==1}">
			             <td><c:out value="待处理"></c:out></td>
			             </c:if>
			             <c:if test="${kg.knowledge.status==2}">
			             <td><c:out value="不处理"></c:out></td>
			             </c:if>
			             <c:if test="${kg.knowledge.status==3}">
			             <td><c:out value="已处理"></c:out></td>
			             </c:if>
			             
	             		<td>${kg.knowledge.TStaff.name}</td>
	             		<td>
	             		<a class="btn-xiugai" href="knowledgeLectureAction!editKnowledgeLecture.action?knowledgeLectureId=${kg.knowledge.knowledgeLectureId}" >修改</a>
	             		<a onclick="return confirm('是否删除？');" class="btn-shanchu" href="knowledgeLectureAction!deleteKnowledgeLecture.action?knowledgeLectureId=${kg.knowledge.knowledgeLectureId}">删除</a>
	             		<a class="btn-xianshi" href="knowledgeLectureAction!knowledgeLectureDetails.action?knowledgeLectureId=${kg.knowledge.knowledgeLectureId}" >详情</a>
	             		</td>
	           		</tr>
	           		</s:iterator>
