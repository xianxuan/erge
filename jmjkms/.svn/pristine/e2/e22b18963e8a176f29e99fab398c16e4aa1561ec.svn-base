...
<body>
  ...
  <div class="main-content clearfix">
    ...
    <div class="container">
       ...
       <h3 class="current-title">住院记录-新增</h3>
                
                <!-- 删除此行注释，在此添加对应的div -->
        <div class="search-content">
        <input name="yemianname" value="06207" type="hidden"/>
           <form  action="inHospitalHistoryAction!findInHospitalHistory.action" method="post" id="registerform1">
            <label>查询选项：</label>
              <select name="flag" id="">
  ...
  <div class="table-content">
			<form id="registerform" action="inHospitalHistoryAction!addInHospitalHistory.action" method="post" onsubmit="return dosubmit()">
            <table class="table table-bordered">
              <tbody>
                 ...
                 <input type="hidden" name="token" value="${token }" />
                  ...
                  <td>
                    <input id="name" type="text"    name="inHospitalHistory.institution" datatype="*" nullmsg="医疗机构名称不可为空"></input>
                  </td> 
                   <td ><span class="required">*</span>病案号</td>
                  <td>
                    <input id="number" type="text"  name="inHospitalHistory.medicalRecord" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"  datatype="*" nullmsg="病案号不可为空" errormsg="输入有误"></input>
                  </td> 
                  <td><span class="required">*</span>入院时间</td>
                  <td>
                    <input type="text" class="startTime" id="Date" onfocus="this.blur()"  name="inHospitalHistory.inDate" value="${currentTime}" onclick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm:ss'})" ></input>
                  </td>
                  <td>出院时间</td>
                  <td>
                    <input type="text"  class="endTime" onfocus="this.blur()"  name="inHospitalHistory.outDate" onclick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm:ss'})"></input>
                  </td>
                ...
                <tr>
                  <td>原因</td>
                  <td colspan="7">
                  <textarea    name="inHospitalHistory.reason" id=""  class="textarea"></textarea>
                  </td>
                </tr>
                <tr>
                  <td>备注</td>
                  <td colspan="7">
                  <textarea    name="inHospitalHistory.note" id=""  class="textarea"></textarea>
                  </td>
                </tr>
                </tbody>
                </table>
                 <div class="btn-content">
                  <input id="submit" type="submit" class="btn" value="提交">
                   <a href="inHospitalHistoryAction!showInHospitalHistory.action" class="btn ">返回</a>
                 </div>
             </form>
        </div>
	 ...
	 <div id="showwindow" class="table-content"" id="button_alert_2">
			...
			<tbody>
				<s:iterator value="listolder" var="lo">
					<tr>
					<td><a href="<%=basePath %>inHospitalHistoryAction!ReOlder.action?healthFileId=${lo.healthFileId}" id="btn_write_off">${lo.name}</a></td>
					<td>${lo.idNum}</td>
				</s:iterator>
			</tbody>
			</table>
