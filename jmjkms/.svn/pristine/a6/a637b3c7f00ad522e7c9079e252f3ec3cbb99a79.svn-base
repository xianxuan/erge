    ...
    <form action="PhysicalExamAction!fuction.action" method="post">
 		<div class="table-content">
    		 ...
    		 <thead>
		         ...
		         <th>家庭住址</th>			              
		         <th>体检详情</th>
		        <c:if test="${issq!=true}"> <th>操作</th></c:if>
		       </tr>
		     </thead>
		     <tbody>
       <%--  <s:action name="PhysicalExamAction!firstIndexsearch" namespace="/" var="fis">
       </s:action> --%>
	        <s:iterator value="vexamList" var="ve">
	        <input type="hidden" name="ids" id="id" value="${ve.physicalExamId}"/>
	      	 <tr>     	 
			      	 <td><fmt:formatDate value='${ve.physicalExamDate}' pattern='yyyy-MM-dd'/></td>
			         <td>${ve.fileName}</td>
			  		 <c:if test="${ve.sex==true}">
						<td>男</td>
			         	</c:if>
			         <c:if test="${ve.sex==false}">
						<td>女</td>
			         	</c:if>
			         <td>
			         <c:out value="${currentTime-(fn:substring(ve.birthDate, 0, 4))}"> </c:out>
			         </td>        
			  		 <td>${fileNum}</td>	         
			         <td>${ve.currentCommunity}</td>
			         <td> <a href="PhysicalExamAction!showPhysicalExam.action?physicalExamId=${ve.physicalExamId}" class="btn-xianshi">查看</a></td>
			 <c:if test="${issq!=true}">
	        <td>  <c:if test="${ve.healthFileType==2 }"><a href="PhysicalExamAction!jumpAddPhysicalExam.action?fileNum=${ve.fileNum}"  class="btn-xinzeng">新增</a> </c:if>
	          <c:if test="${ve.healthFileType==2 }">  <a href="PhysicalExamAction!deletePhysicalExam.action?physicalExamId=${ve.physicalExamId}" onclick="return confirm('是否删除？');" class="btn-shanchu">删除</a> </c:if>
	         </td>
	        </c:if>
	        </tr>
	       </s:iterator>
