    ...
    <div class="container">
      ...
      <!-- 删除此行注释，在此添加对应的div -->
       ...
       <input name="yemianname" value="06204" type="hidden"/>
            <form id="registerform1" action="dualReferralRecordAction!findDualReferralRecord.action" method="post">
            <label>查询选项：</label>
              <select name="flag" id="">
       ...
       <div class="table-content">
           ...
           <table class="table table-bordered">
             <tbody>
               ...
               <input type="hidden" name="healthFileId" value="${healthFile.healthFileId}" /> 
                 ...
                 <td><input class="startTime" type="text" id="Date" onfocus="this.blur()"  name="dualReferralRecord.outTime" onclick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm:ss'})" ></td>
           ...
           <div class="btn-content">
             ...
             <a href="dualReferralRecordAction!showDualReferralRecord.action" class="btn">返回</a>
           </div>
           </form>
       </div>
	 ...
	 <div id="showwindow" class="table-content"" id="button_alert_2">
			...
			<tbody>
				<s:iterator value="listolder" var="lo">
					<tr>
					<td><a href="<%=basePath %>dualReferralRecordAction!ReOlder.action?healthFileId=${lo.healthFileId}" id="btn_write_off">${lo.name}</a></td>
					<td>${lo.idNum}</td>
				</s:iterator>
			</tbody>
			</table>
