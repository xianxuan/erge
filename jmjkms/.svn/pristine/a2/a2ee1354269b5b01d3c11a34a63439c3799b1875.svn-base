						...
						<tbody>
							<tr><input type="hidden" name="token" value="${token }" />
								<input type="hidden" name="healthFile.healthFileId"
									value="${healthFileCloudSingle.healthFileId}">
								 <input type="hidden" name="healthFile.id"
									value="${healthFileCloudSingle.id}">
								<td>身份证号</td>
								<td><lable>${healthFileCloudSingle.idNum}</lable></td>
								<input type="hidden"  name="healthFile.idNum" value="${healthFileCloudSingle.idNum}" >
								<td>居民档案编号</td>
								<td><lable>${healthFileCloudSingle.fileNum} </lable></td>
								<input type="hidden" name="healthFile.fileNum" value="${healthFileCloudSingle.fileNum}"> 
							</tr>
							 <tr>
								<td>姓名</td>

								<td><lable>${healthFileCloudSingle.name}</lable></td>
								<input type="hidden" name="healthFile.name" value="${healthFileCloudSingle.name}" >

								 <td>本人电话</td>
								<td><label>${healthFileCloudSingle.iPhone}</label></td>
								<input  name="healthFile.IPhone"value="${healthFileCloudSingle.iPhone}" type="hidden"> 
							</tr> 
							 <tr>
									...
									style="width: 170px;" datatype="*" nullmsg="责任医生不可为空" >
										...
										<s:iterator value="specialDoctor" var="hh">
											<option value="${hh.staffId}"
												<c:if test="${healthFileCloudSingle.doctorID eq hh.staffId}">selected</c:if>><c:out
								...
								<td>建档日期</td>
								<td><input  type="text" name="healthFile.fileDate" id="datetime"  value="${healthFileCloudSingle.fileDate}" onclick="laydate({istime: true, format: 'YYYY-MM-DD'})"></td>
							</tr>
							<tr>
								<td>乡镇（街道）名称</td>
								<td><input type="text" name="healthFile.currentOffice"
									value="${healthFileCloudSingle.currentOffice}"  maxlength="15" datatype="*" nullmsg="姓名不可为空"></td>
								<td>建档人</td>
								<td><lable>${healthFileCloudSingle.filePerson}</lable>
									<input type="hidden"
									name="healthFile.filePerson" value="${healthFileCloudSingle.filePerson}">
                          </td>
							</tr>
							<tr>
								<td>村（居）委员名称</td>
								<td><input type="text"
									value="${healthFileCloudSingle.currentResidentCommittee}"
								...
								<td><select name="healthFile.enterPeople" id=""
									...
									style="width: 170px;">
										<s:iterator value="staffs" status="n" var="number">
											...
											<option value="${number.name}" <c:if test="${healthFileCloudSingle.enterPeople eq number.name}">selected</c:if>>${number.name}</option>
										</s:iterator>
								</select></td>
								</c:if>
								<c:if test="${flag==true}">
                        ...
                        <lable>${tstaff.name} </lable>
							...
							<tr>
								...
								<td colspan="3"><c:out value="${healthFileCloudSingle.otherProvince}"></c:out>
									...
									value="${healthFileCloudSingle.otherProvince}"> <c:out
										value="${healthFileCloudSingle.otherCity}"></c:out> <input type="hidden"
									name="healthFile.otherCity" value="${healthFileCloudSingle.otherCity}">
									<c:out value="${healthFileCloudSingle.otherCounty}"></c:out> <input
									type="hidden" name="healthFile.otherCounty"
									value="${healthFileCloudSingle.otherCounty}"> <c:out
										value="${healthFileCloudSingle.otherCommunity}"></c:out> <input
									type="hidden" name="healthFile.otherCommunity"
									value="${healthFileCloudSingle.otherCommunity}"></td>

							</tr>
							<tr>
								<td>现住址</td>
								<td colspan="3"><c:out
										value="${healthFileCloudSingle.currentProvince}"></c:out> <input
									type="hidden" name="healthFile.currentProvince"
									value="${healthFileCloudSingle.currentProvince}"> <c:out
										value="${healthFileCloudSingle.currentCity}"></c:out> <input
									type="hidden" name="healthFile.currentCity"
									value="${healthFileCloudSingle.currentCity}"> <c:out
										value="${healthFileCloudSingle.currentCounty}"></c:out> <input
									type="hidden" name="healthFile.currentCounty"
									value="${healthFileCloudSingle.currentCounty}"> <c:out
										value="${healthFileCloudSingle.currentCommunity}"></c:out> <input
									type="hidden" name="healthFile.currentCommunity"
									value="${healthFileCloudSingle.currentCommunity}"></td>
							</tr>
							<tr>
								<td>照片</td>
								<td colspan="3">
                          <input type="hidden" name="healthFile.pic"  value="${healthFileCloudSingle.pic}">
							<img id="second" src="${healthFileCloudSingle.pic}" onerror="this.src='/jmjkms/images/healthfile_default_pic.jpg'" width="100px" height="100px">
                      ...
                      <tr>
                        ...
                        <td>
                          <select name="healthFile.sex" id="sex">
                            <option value=false <c:if test="${healthFileCloudSingle.sex == false}">selected</c:if>>男</option>
                            <option value=true <c:if test="${healthFileCloudSingle.sex == true}">selected</c:if>>女</option>
                            
                          </select>
                        </td>
                        <td>出生日期</td>
                         <td><input name="healthFile.birthDate" type="text" id="birthDate" value="${healthFileCloudSingle.birthDate}" onclick="laydate({istime: true, format: 'YYYY-MM-DD'})" ></td>
                      </tr>
                      <tr>
                        
                        <td>所在社区医院</td>
                        <td>
                        <lable>${communityHospital.communityHospital}</lable>
                        ...
                        <input type="hidden" name="healthFile.hospital" value="${communityHospital.communityHospital}">
								...
								<td>建档单位</td>
								<td><lable>${healthFileCloudSingle.workUnit}</lable></td>
								<input type="hidden" name="healthFile.workUnit"
									value="${healthFileCloudSingle.workUnit}">

							</tr>
						 	<tr>
								<td>联系人姓名</td>
								<td><input name="healthFile.otherName"
									value="${healthFileCloudSingle.otherName}" type="text" maxlength="5"></td>
								<td>联系人电话</td>
								<td><input id="dianhua1" name="healthFile.otherPhone"
									value="${healthFileCloudSingle.otherPhone}" type="text" datatype="n8-11"
							...
							<tr>
								...
								<td><select name="healthFile.permanentType" id="">
										<option value="0"
											<c:if test="${healthFileCloudSingle.permanentType eq '户籍'}">selected</c:if>>户籍</option>
										<option value="1"
											<c:if test="${healthFileCloudSingle.permanentType eq '非户籍'}">selected</c:if>>非户籍</option>
								</select></td>
								<td>民族</td>
								<td><select name="healthFile.nation" id="">
										<s:iterator value="mz" status="n" var="number">
											<option value="${number.name}"
												<c:if test="${healthFileCloudSingle.nation eq number.name}">selected</c:if>>${number.name}</option>
										</s:iterator>
								</select></td>
							</tr>
							<tr>
								...
								<td><select name="healthFile.bloodTypr" id="">
										<option value="0"
											<c:if test="${healthFileCloudSingle.nation eq 'A型'}">selected</c:if>>A型</option>
										<option value="1"
											<c:if test="${healthFileCloudSingle.nation eq 'B型'}">selected</c:if>>B型</option>
										<option value="2"
											<c:if test="${healthFileCloudSingle.nation eq 'O型'}">selected</c:if>>O型</option>
										<option value="3"
											<c:if test="${healthFileCloudSingle.nation eq 'AB型'}">selected</c:if>>AB型</option>
										<option value="4"
											<c:if test="${healthFileCloudSingle.nation eq '不详'}">selected</c:if>>不详</option>
								</select></td>
								<td>RH阴性</td>
								<td><select name="healthFile.rhNegative" id="">
										<option value="0"
											<c:if test="${healthFileCloudSingle.rhNegative eq '0'}">selected</c:if>>否</option>
										<option value="1"
											<c:if test="${healthFileCloudSingle.rhNegative eq '1'}">selected</c:if>>是</option>
										<option value="2"
											<c:if test="${healthFileCloudSingle.rhNegative eq '2'}">selected</c:if>>不详</option>
								</select></td>
							</tr>
							<tr>
								<td>职业</td>
								<td colspan="3"><select name="healthFile.job" id="">
										<s:iterator value="zy" status="n" var="number">
											<option value="${number.name}"
												<c:if test="${healthFileCloudSingle.job eq number.name}">selected</c:if>><c:out
							...
							<tr>
								...
								<td><select name="healthFile.maritalStatus" id="">
										<s:iterator value="hyzk" status="n" var="number">
											<option value="${number.name}"
												<c:if test="${healthFileCloudSingle.maritalStatus eq number.name}">selected</c:if>>${number.name}</option>
										</s:iterator>
								</select></td>
								<td>文化程度</td>
								<td><select name="healthFile.eduLevel" id="">
										<s:iterator value="whcd" status="n" var="number">
											<option value="${number.name}"
												<c:if test="${healthFileCloudSingle.eduLevel eq number.name}">selected</c:if>><c:out
													value="${number.name}"></c:out></option>
										</s:iterator>
								</select></td>
							</tr>
							<tr>
								...
								<td colspan="3"><s:iterator value="payMethodList"
									...
									<s:if test="str.indexOf('城镇职工医疗保险')==-1">
							...
							<tr>
								...
								<td colspan="3"><input type="checkbox" class="check-wu"
									...
									<s:iterator value="allergyNameList" status="n"  var="number">
							<input type="checkbox" name="choiceAllergyName" id="payment"
							value="number"
							 <s:iterator value="choiceAllergyName" status="n"  var="number1">
								<c:if test="${number==number1}"> 
								checked
								</c:if>
							</s:iterator>
							/> 
							...
							<tr>
								...
								<td colspan="3"><input type="checkbox" valclass="check-wu"
									...
									name="healthFile.exposeNote" value="${healthFileCloudSingle.exposeNote}"
									type="text">  
									<s:iterator value="exposeNameList" status="n"  var="number">
							<input type="checkbox" name="choiceExposeName" id="payment"
							value="number"
							 <s:iterator value="choiceExposeName" status="n"  var="number1">
								<c:if test="${number==number1}"> 
								checked
								</c:if>
							</s:iterator>
							...
							<tr>
								<td>身高</td>
								<c:choose>
                                       	<c:when test="${healthFileCloudSingle.height eq 0.0}">
                                       	...
                                       	<c:otherwise> 
                                       		<td><input id="shengao" onkeyup="Num(this)" type="text"
											name="healthFile.height" value="${healthFileCloudSingle.height}"
											class="w40" onkeyup="value=value.replace(/[^\d.]/g,'')"
											maxlength="3">cm</td>
                                       	</c:otherwise>  
                                </c:choose>
								...
								<td>体重</td>
								<c:choose>
                                       	<c:when test="${healthFileCloudSingle.weihth eq 0.0}">
                                       	...
                                       	<c:otherwise> 
                                       		<td><input id="tizhong" onkeyup="Num(this)" type="text"
											name="healthFile.weihth" value="${healthFileCloudSingle.weihth}"
											class="w40" onkeyup="value=value.replace(/[^\d.]/g,'')"
											maxlength="3">kg</td>
                                       	</c:otherwise>  
                                </c:choose>
							 ...
							 <tr>
								<td>腰围</td>
								<c:choose>
                                       	<c:when test="${healthFileCloudSingle.waistline eq 0.0}">
                                       	...
                                       	<c:otherwise> 
                                       		<td><input id="yaowei" onkeyup="Num(this)" type="text"
											name="healthFile.waistline" value="${healthFileCloudSingle.waistline}"
											class="w40" onkeyup="value=value.replace(/[^\d.]/g,'')"
											maxlength="3">cm</td>
                                       	</c:otherwise>  
                                </c:choose>
								...
								<td>臀围</td>
								<c:choose>
                                       	<c:when test="${healthFileCloudSingle.hipline eq 0.0}">
	                                     <td><input id="tunwei" onkeyup="Num(this)" type="text"
                                       	...
                                       	<c:otherwise> 
                                       		<td><input id="tunwei" onkeyup="Num(this)" type="text"
											name="healthFile.hipline" value="${healthFileCloudSingle.hipline}"
											class="w40" onkeyup="value=value.replace(/[^\d.]/g,'')"
											maxlength="3">cm</td>
                                       	</c:otherwise>  
                                </c:choose>
							...
							<tr>
								<td>收缩压</td>
									<c:choose>
                                       	<c:when test="${healthFileCloudSingle.systolicBloodPressure eq 0.0}">
	                                     <td><input onkeyup="Num(this)" type="text"
                                       	...
                                       	<c:otherwise> 
                                       		<td><input onkeyup="Num(this)" type="text"
												value="${healthFileCloudSingle.systolicBloodPressure}"
												name="healthFile.systolicBloodPressure" class="w40"
												onkeyup="value=value.replace(/[^\d.]/g,'')" maxlength="3">mmgh
											</td>
                                       	</c:otherwise>  
								...
								<td>舒张压</td>
											<c:choose>
                                       	<c:when test="${healthFileCloudSingle.diastolicBloodPressure eq 0.0}">
	                                     <td><input onkeyup="Num(this)" type="text"
                                       	...
                                       	<c:otherwise> 
                                       		<td><input onkeyup="Num(this)" type="text"
												value="${healthFileCloudSingle.diastolicBloodPressure}"
												name="healthFile.diastolicBloodPressure" class="w40"
												onkeyup="value=value.replace(/[^\d.]/g,'')" maxlength="3">mmgh
											</td>
                                       	</c:otherwise>  
							...
							<tr>
								...
								<td colspan="3"><input id="NFCbianma" type="text"
									value="<c:out value="${healthFileCloudSingle.nfc}"></c:out>"
          ...
          <tbody> 
          		<tr>
							...
							<tr>
								...
								<select name="tlifestyle.exerciseFrequency" id="">
									...
									<s:iterator value="tydlplList" var="ty"> 
                                      		 <option value="${ty.name}" <c:if test="${ty.name eq lifestyle.exerciseFrequency}">selected=selected</c:if>>${ty.name}</option>
                                       </s:iterator>
								<optionflue="良好" <c:if test="${'良好' eq lifestyle.exerciseFrequency}">selected=selected</c:if> >良好</option>
							</select>
								...
								<td>
									...
									name="tlifestyle.exerciseTime" value="${lifestyle.exerciseTime} ">min
								</td>
								</tr>
								<tr>
								<td>坚持锻炼时间</td>
								<td><input type="text" class="w40"   maxlength="5"
									name="tlifestyle.allExerciseTime" value="${lifestyle.allExerciseTime}"> min</td>
								<td>锻炼方式</td>
								<td><select name="tlifestyle.exerciseType" id="">
										<option value=" ">请选择</option>
										
										<s:iterator value="dlfsList" var="dl"> 
                                      		 <option value="${dl.name}" <c:if test="${dl.name eq lifestyle.exerciseType}">selected=selected</c:if>>${dl.name}</option>
                                       </s:iterator>
										
									<option value="良好" <c:if test="${'良好' eq lifestyle.exerciseType}">selected=selected</c:if> >良好</option>
								</select> </td>
							</tr>
							<tr>
								<td>饮食习惯</td>
								<td><select name="tlifestyle.dietaryHabit" id="">
										<option value=" ">请选择</option>
										<option value="良好" <c:if test="${'良好' eq lifestyle.dietaryHabit}">selected=selected</c:if> >良好</option>
									<s:iterator value="ysxgList" var="ysxg"> 
                                      		 <option value="${ysxg.name}" <c:if test="${ysxg.name eq lifestyle.dietaryHabit}">selected=selected</c:if> >${ysxg.name}</option>
                                       </s:iterator>
								</select>
								</td>
								<td>吸烟状况</td>
								<td><select name="tlifestyle.smokingStatus" id="">
										...
										<s:iterator value="xyzkList" var="xyzk"> 
                                      		 <option value="${ xyzk.name}" <c:if test="${ xyzk.name eq lifestyle.smokingStatus}">selected=selected</c:if>>${ xyzk.name}</option>
                                       </s:iterator>
										<option value="良好" <c:if test="${'良好' eq lifestyle.smokingStatus}">selected=selected</c:if> >良好</option>
								</select></td>
								</tr>
								<tr>
								<td>日饮酒量平均</td>
								<c:choose>
                                       	<c:when test="${lifestyle.smokingOneday eq 0.0}">
                                       	<td>
										...
										name="tlifestyle.smokingOneday" > 两</td>
                                       	...
                                       	<td>
										<input type="text" class="w40"  maxlength="5" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"
										name="tlifestyle.smokingOneday" value="${lifestyle.smokingOneday}"> 两</td>
                                       	</c:otherwise>  
                                </c:choose>
								
								<td>开始吸烟年龄</td>
								<c:choose>
                                       	<c:when test="${lifestyle.startsmokingAge eq 0}">
									...
									name="tlifestyle.startsmokingAge" > 岁</td>
                                       	...
                                       	<c:otherwise> 
                                     <td><input type="text" class="w40" maxlength="3" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"
									name="tlifestyle.startsmokingAge" value="${lifestyle.startsmokingAge}"> 岁</td>
                                       	</c:otherwise>  
                                </c:choose>
							...
							<tr>
								<td>戒烟年龄</td>
								<c:choose>
                                       	<c:when test="${lifestyle.giveupSmokingAge eq 0.0}">
                                       	<td><input type="text" class="w40" maxlength="3" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"
									name="tlifestyle.giveupSmokingAge" > 岁</td>
                                       	</c:when>
                                       	<c:otherwise> 
                                       	<td><input type="text" class="w40" maxlength="3" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"
									name="tlifestyle.giveupSmokingAge" value="${lifestyle.giveupSmokingAge}"> 岁</td>
                                       	</c:otherwise>  
                                </c:choose>
								...
								<td><select name="tlifestyle.drinkingFrequency" id="">
										...
										<s:iterator value="yjplList" var="yj"> 
                                      		 <option value="${yj.name}" <c:if test="${yj.name eq lifestyle.drinkingFrequency}">selected=selected</c:if>>${yj.name}</option>
                                       </s:iterator>
										<option value="良好" <c:if test="${'良好' eq lifestyle.drinkingFrequency}">selected=selected</c:if> >良好</option>
								</select></td>
								</tr>
								<tr>
								<td>日饮酒量</td>
								<c:choose>
                                       	<c:when test="${lifestyle.monthlyDrinking eq 0.0}">
									...
									name="tlifestyle.monthlyDrinking" > 两</td>
                                       	...
                                       	<c:otherwise> 
                                       <td><input type="text" class="w40" maxlength="5" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"
									name="tlifestyle.monthlyDrinking" value="${lifestyle.monthlyDrinking}"> 两</td>
                                       	</c:otherwise>  
                                </c:choose>
								...
								<td><select name="tlifestyle.abstinence" id="">
										<option value=" ">请选择</option>
										<option value="是" <c:if test="${'是' eq lifestyle.abstinence}">selected=selected</c:if> >  是</option>
										<option value="否"<c:if test="${'否' eq lifestyle.abstinence}">selected=selected</c:if> >  否</option>
								</select>
						...
						<tr>
								<td>开始饮酒年龄</td>
								<c:choose>
                                       	<c:when test="${lifestyle.startDrinkingAge eq 0.0}">
                                       <td><input type="text" class="w40" maxlength="3" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"
									name="tlifestyle.startDrinkingAge" >岁</td>
                                       	</c:when>
                                       	<c:otherwise> 
                                       <td><input type="text" class="w40" maxlength="3" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"
									name="tlifestyle.startDrinkingAge" value="${lifestyle.startDrinkingAge}">岁</td>
                                       	</c:otherwise>  
                                </c:choose>
								...
								<td colspan="1"><select
									name="tlifestyle.recentlyYearDrinking" id="">
										<option value="">请选择</option>
										<option value="良好" <c:if test="${'良好' eq lifestyle.recentlyYearDrinking}">selected=selected</c:if> >良好</option>
								</select> </td>
								</tr>
								<tr>
								<td>饮酒种类</td>
								<td colspan="1"><select name="tlifestyle.drinkingType" id="">
										...
										<s:iterator value="yjzlList" var="yz"> 
                                      		 <option value="${yz.name}" <c:if test="${yz.name eq lifestyle.drinkingType}">selected=selected</c:if> >${yz.name}</option>
                                       </s:iterator>
										<option value="良好" <c:if test="${'良好' eq lifestyle.drinkingType}">selected=selected</c:if> >良好</option>
								</select></td>
								...
								<td colspan="1">
									<div class="jia">
                                                ...
                                                <input maxlength="30" type="text"   class="gai"  name="tlifestyle.occupationalDisease" value="${lifestyle.occupationalDisease}" >
                                            </div>
								</td>
								</tr>
								<tr>
								<td>毒物种类</td>
								<td>
									<input maxlength="20"  type="text" class="w40" name="tlifestyle.dust" value="${lifestyle.dust}">
								</td>
								<td>防护措施${lifestyle.dustFence}</td>
								<td colspan="1">
								<div class="jia">
										<select     class="di">
										...
										<input maxlength="30" type="text"   class="gai" name="tlifestyle.dustFence" value="${lifestyle.dustFence}" >
									</div>
								...
								<%-- <div class="jia">
										...
										<input type="text"    class="gai" name="tlifestyle.dustFence" value="${lifestyle.dustFence}" >
									</div> --%>
							...
							<tr>
								...
								<td>
									...
									name="tlifestyle.radiogen" value="${lifestyle.radiogen}">
									 </td>
								<td>防护措施</td>
								<td>
								<div class="jia">
										...
										<input maxlength="30" type="text"   class="gai" name="tlifestyle.radiogenFence" value="${lifestyle.radiogenFence}" >
									</div>
								...
								<td>
									<input type="text" class="w40" maxlength="20"
									name="tlifestyle.physicalFactor" value="${lifestyle.physicalFactor}">
									</td>
								<td>防护措施</td>
								<td>
								<div class="jia">
										...
										<input maxlength="30" type="text"   class="gai" name="tlifestyle.physicalFactorFence" value="${lifestyle.physicalFactorFence}" >
									</div>
								</td>
							</tr>
							<tr>
								...
								<td><input type="text" class="w40"  maxlength="20"
									name="tlifestyle.chemicalSubstances" value="${lifestyle.chemicalSubstances}">
								...
								<div class="jia">
										...
										<input  type="text"    class="gai" name="tlifestyle.chSubFence" value="${lifestyle.chSubFence}" >
									</div>
								</td>
								</tr>
								<tr>
								<td>其他</td>
								<td><input maxlength="30" type="text" class="w40" name="tlifestyle.other" value="${lifestyle.other}">
									</td>
								<td>防护措施</td>
								<td>
								<div class="jia">
										...
										<input maxlength="30" type="text"   class="gai" name="tlifestyle.otherFence" value="${lifestyle.otherFence}" >
									</div>
							...
							<s:iterator value="pastShouShuList" status="n"
								var="number">
								<tr class="dyna-form">
							...
							<s:iterator value="pastWaiShangList" status="n"
								var="number">
								<tr class="dyna-form">
									<td colspan="2"><input type="text"
							...
							<s:iterator value="pastShuXieList" status="n" var="number">
								<tr class="dyna-form">
									<td colspan="2"><input type="text"
						...
						<tr>
							...
							<tr>
								...
								<td colspan="3"><input type="checkbox" name="choiceFather"
									...
									name="healthFile.one" value="${healthFileCloudSingle.one}"></td>
							</tr>
							<tr>
								<td>母亲</td>
								<td colspan="3"><input type="checkbox" name="choiceMother"
									...
									name="healthFile.two" value="${healthFileCloudSingle.two}"></td>
							</tr>
							<tr>
								<td>兄弟姐妹</td>
								<td colspan="3"><input type="checkbox" name="choiceBroSis"
									...
									name="healthFile.three" value="${healthFileCloudSingle.three}"></td>
							</tr>
							<tr>
								<td>子女</td>
								<td colspan="3"><input type="checkbox" name="choiceChild"
									...
									name="healthFile.four" value="${healthFileCloudSingle.four}"></td>
							</tr>
							<tr>
								<td>遗传病史</td>
								<td colspan="3"><input type="text"
									name="healthFile.geneticHistory"
									value="${healthFileCloudSingle.geneticHistory}" placeholder="无">(注明疾病名称)</td>
							</tr>
							<tr>
								<td>残疾情况</td>
								<td colspan="3"><input type="checkbox" class="check-wu"
									...
									value="${healthFileCloudSingle.disabilitity}" type="text"> <s:iterator value="disabilityNameList" status="n"  var="number">
                        
							<input type="checkbox" name="choiceDisabilityName" id="payment"
							value="number"
							 <s:iterator value="choiceDisabilityName" status="n"  var="number1">
								<c:if test="${number==number1}"> 
								checked
								</c:if>
							</s:iterator>
							...
							<tr>
								...
								<td><select name="healthFile.kitVenFac" id="">
										<s:iterator value="cfpfcs" status="n" var="number">
											<option value="${number.name}"
												<c:if test="${healthFileCloudSingle.kitVenFac eq number.name}">selected</c:if>><c:out
													value="${number.name}"></c:out></option>
										</s:iterator>
								</select></td>
								<td>燃料类型</td>
								<td><select name="healthFile.fuelType" id="">
										<s:iterator value="rllx" status="n" var="number">
											<option value="${number.name}"
												<c:if test="${healthFileCloudSingle.fuelType eq number.name}">selected</c:if>><c:out
													value="${number.name}"></c:out></option>
										</s:iterator>
								</select></td>
							</tr>
							<tr>
								...
								<td><select name="healthFile.drinkWater" id="">
										<s:iterator value="ys" status="n" var="number">
											<option value="${number.name}"
												<c:if test="${healthFileCloudSingle.drinkWater eq number.name}">selected</c:if>><c:out
													value="${number.name}"></c:out></option>
										</s:iterator>
								</select></td>
								<td>厕所</td>
								<td><select name="healthFile.toilet" id="">
										<s:iterator value="cs" status="n" var="number">
											<option value="${number.name}"
												<c:if test="${healthFileCloudSingle.toilet eq number.name}">selected</c:if>><c:out
													value="${number.name}"></c:out></option>
										</s:iterator>
								</select></td>
							</tr>
								...
								<td colspan="3"><select name="healthFile.livestockBar"
									id="">
										<s:iterator value="qcl" status="n" var="number">
											<option value="${number.name}"
												<c:if test="${healthFileCloudSingle.livestockBar eq number.name}">selected</c:if>><c:out
													value="${number.name}"></c:out></option>
										</s:iterator>
								</select></td>
							...
							<tr>
								...
								<td colspan="3"><select name="healthFile.crowdClass" id="">
										<option value="0"
											<c:if test="${healthFileCloudSingle.crowdClass eq 0}">selected</c:if>>健康人群</option>
										<option value="1"
											<c:if test="${healthFileCloudSingle.crowdClass eq 1}">selected</c:if>>慢病高危人群</option>
										<option value="2"
											<c:if test="${healthFileCloudSingle.crowdClass eq 2}">selected</c:if>>慢病患者人群</option>
								</select></td>
							</tr>  
        ...
        <table class="table table-bordered">
          <tbody>
						<tr><td colspan="8" class="title-td">用药记录</td></tr>         
							...
							<s:iterator value="MedicalRecordList" status="n" var="number">
								<tr class="dyna-form">
									...
									<td class="w50"><input class="w50" type="text" name="fileMedicalListBegin"
										value="${number.beginDate}"
										onclick="laydate({istime: true, format: 'YYYY-MM-DD'})"></td>
									<!-- <td class="w50"><input class="w50" type="text" name="fileMedicalListStop"
										value="${number.stopDate}"
										onclick="laydate({istime: true, format: 'YYYY-MM-DD'})"></td>
										-->
									<td colspan="1" ><input class="w50" type="text"
	...
	<script type="text/javascript">
		...
		window.onload = function() {
			...
			$
					.ajax({
						type : "post",
						url : "healthFileAction!getDisease.action?healthCloudId="+${healthFileCloudSingle.id},
	...
	<%-- <script type="text/javascript">
		...
		window.onload = function() {
			...
			$
					.ajax({
						type : "post",
						url : "healthFileAction!getDisease.action?healthFileId="+ ${healthFileCloudSingle.healthFileId}+"&healthCloudId="+${healthFileCloudSingle.id} ,
						dataType : "json",
						success : function(data) {
							alert(2);
							if (data.length == 0) {
...
<!--<script type="text/javascript">
 $("form").submit(function(){
    	var startTime0=$(".fileMedicalListBegin").eq(0).val();
    	var startTime1=$(".fileMedicalListBegin").eq(1).val();
    	var endTime0=$(".fileMedicalListStop").eq(0).val();
    	var endTime1=$(".fileMedicalListStop").eq(1).val();
    	if(startTime0 != ""&&endTime0 != ""){
    	if(startTime0>endTime0){
      	 	$(".fileMedicalListBegin").eq(0).val("");
       		$(".fileMedicalListStop").eq(0).val("");
       		alert("查询时间输入错误");
       		return false;
       	}
       	}
    	...
    	if(startTime1>endTime1){
      	 	$(".fileMedicalListBegin").eq(1).val("");
       		$(".fileMedicalListStop").eq(1).val("");
       		alert("查询时间输入错误");
       		return false;
       	}
       	}
