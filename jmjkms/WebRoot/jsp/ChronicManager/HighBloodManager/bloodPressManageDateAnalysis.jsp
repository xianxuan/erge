    ...
    <div class="container">
     ...
     <input name="yemianname" value="02204" type="hidden"/>
          <div class="btn-content">
            <a href="chronicManagerAction!getBloodPressFromCloud.action?bloodForward=0&healthFileId=${healthFile.healthFileId }" class="btn">数据分析</a>
       ...
       <form id="registerform" action="chronicManagerAction!bloodPressDataAnalysis.action" method="post" >
           ...
           <s:radio name="searchType" cssClass="type" list="#{'0':'按时间查询','1':'按数量查询'}"  label="请选择查询类型" ></s:radio>
            <label><input type="hidden" name="healthFileId" value="${healthFile.healthFileId }" /></label>
               ...
               <div id="time" style="display: inline-block;">
                  ...
                  <label >最近信息条数：</label>
                  <input type="text" class="w40" id="num" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')" name="totalSize"   value="${ totalSize}">
                  </div>
                  <input type="submit" class="btn" id="sub" value="查询">
                </div>
                </form>    
       <!-- 表单表格 start -->
