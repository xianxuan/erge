...
<body>
  ...
  <!-- 主容器 start -->
    <div class="container">
      <h3 class="current-title">糖尿病康复计划</h3>
 ...
 <!-- 搜索容器 start -->
      ...
      <input name="yemianname" value="02205" type="hidden"/>
        <form id="registerform" action="chronicManagerAction!searchRecoveryPlan.action" method="post">
          ...
          <div class="search-item">
           <label>计划名称：</label>
            <input id="planName"   maxlength='20' value="${planName }" type="text" name="planName"  errormsg="不能输入特殊字符！" class="w150" > 
          </div>
          <div class="search-item">
            <label>计划状态：</label>
          ...
          <div class="btn-content">
            ...
            <a class="btn btn-tianjia" id="tianjia" href="chronicManagerAction!addRecoveryPlan.action?chronicId=${chronicId} " class="btn btn-xinzeng">添加计划</a>
          </div>
        </form>
 ...
 <div class="table-content">
   <table class="table table-bordered">
     ...
     <s:iterator value="recoveryOldList" var="rec">
       <tr>
         <td>${rec.TRecoveryPlan.planName }</td>
         <td>${rec.TRecoveryPlan.TRecoveryScheme.schemeName }</td>
         <td>
         	<fmt:formatDate value='${rec.TRecoveryPlan.beginTime }' pattern='yyyy-MM-dd'/>
         </td>
         <td>
         	<fmt:formatDate value='${rec.TRecoveryPlan.overTime }' pattern='yyyy-MM-dd'/>
         </td>
         <td>
         	<fmt:formatDate value='${rec.TRecoveryPlan.constituteTime }' pattern='yyyy-MM-dd'/>
         </td>
         <td>${rec.TRecoveryPlan.TStaff.name }</td>
   ...
   <div class="btn-content">
    <a class="btn btn-xianshi" href="chronicManagerAction!getAllDiabeteManager.action">返回</a>
   </div>
 </div>
 <!-- 表格容器 end -->
