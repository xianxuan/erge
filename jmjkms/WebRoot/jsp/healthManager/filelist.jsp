         ...
         <option value="${s.name}" <c:if test="${s.name eq searchbean.enterPeople}">selected</c:if>>${s.name}</option>
			...
			<div class="table-content">
				<table class="table table-bordered">
					...
					<tbody>
					<s:iterator value="vOldStaffs" var="hfcl"  status="n">
						<tr>
							<td>${hfcl.HName}</td>
							...
							<td class="canji"><a class="btn-xinzeng" href="HealthManagerAction!toUploadData.action?healthFileId=${ hfcl.healthFileId}">采集</a></td>
							<td class="dangan"><a class="btn-xianshi" href="HealthManagerAction!getDataFromCloud.action?healthFileId=${ hfcl.healthFileId}">查看</a></td>
						</tr>
                    </s:iterator>
					</tbody>
				</table>
