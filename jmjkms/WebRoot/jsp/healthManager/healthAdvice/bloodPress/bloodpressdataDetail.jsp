      ...
      <input name="yemianname" value="02202" type="hidden"/>
         <div class="search-content">
          <div class="btn-content">
            <a href="HealthManagerAction!getBloodPressFromCloud.action?bloodForward=0&healthFileId=${healthFile.id }" class="btn btn-default">数据分析</a>
            <a href="HealthManagerAction!getBloodPressFromCloud.action?bloodForward=1&healthFileId=${healthFile.id }" class="btn btn-default">趋势分析</a>
            <a href="HealthManagerAction!getBloodPressFromCloud.action?bloodForward=2&healthFileId=${healthFile.id }" class="btn ">详细数据</a>
         ...
         <form id="registerform" action="HealthManagerAction!bloodPressDataDetail.action" method="post">
          ...
          <s:iterator value="bloodPresseList"  var="bpl" status="n">
           <tr>
            ...
            <c:if test="${bpl.takeMed!=1}"><td>未服</td></c:if>
             <c:if test="${bpl.takeMed==1}"><td>${bpl.medName}</td></c:if>
             <c:if test="${bpl.arm==0 }" ><td>左手</td></c:if>
              <c:if test="${bpl.arm==1 }" ><td>右手</td></c:if>
              <c:if test="${bpl.arm!=0&&bpl.arm!=1 }" ><td></td></c:if>
             <td>${bpl.sysPre }</td>
             <td>${bpl.diaPre }</td>
             <td>${bpl.pulse }</td>
             <td>${bpl.meaTime }</td>
            <td class="caozuo">
               <a class="btn-xiugai" href="HealthManagerAction!toEditBPCloud.action?healthFileId=${healthFile.id }&cloudId=${bpl.id}&meaState=${bpl.meaState}&diaPre=${bpl.diaPre }&sysPre=${bpl.sysPre }&pulse=${bpl.pulse }&arm=${bpl.arm }&devID=${bpl.devID }&takeMed=${bpl.takeMed }&medName=${bpl.medName }&meaTime=${bpl.meaTime}">修改</a>
               <a class="btn-shanchu" href="HealthManagerAction!deleteBloodPressDataCloud.action?healthFileId=${healthFile.id }&cloudId=${bpl.id }" onclick="return confirm('确认删除')">删除</a>
              </td>
           </tr>
          </s:iterator>
         </tbody>
