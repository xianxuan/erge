...
<body>
  ...
  <div class="main-content clearfix">
    ...
    <!-- 主容器 start -->
    <div class="container">
        <h3 class="current-title">血糖档案 康复计划</h3>
         <input name="yemianname" value="02202" type="hidden"/>
       <div class="search-content">
          <div class="btn-content">
            <a href="HealthManagerAction!getBloodSugerFromCloud.action?bloodForward=0&healthFileId=${healthFileId }" class="btn btn-default">数据分析</a>
            <a href="HealthManagerAction!getBloodSugerFromCloud.action?bloodForward=1&healthFileId=${healthFileId } " class="btn btn-default">趋势分析</a>
            <a href="HealthManagerAction!getBloodSugerFromCloud.action?bloodForward=2&healthFileId=${healthFileId }" class="btn btn-default">详细数据</a>
            <%-- <a href="HealthManagerAction!getBloodSugerFromCloud.action?bloodForward=3&healthFileId=${healthFileId }" class="btn ">康复计划</a> --%>
          </div>
      </div>
      <!-- 搜索容器 start -->
 <div class="search-content">
   <form action="HealthManagerAction!searchBloodPressRecoverPlan.action" method="post" >
    <div class="search-item">
       ...
       <input type="hidden" name="diseaseName" value="糖尿病" />
        <label>起始时间：</label>
            <input  name="startTime" onfocus="this.blur()" id="startTime" type="text" class="w150" value="<fmt:formatDate value="${startTime }" pattern="yyyy-MM-dd"/>" onclick="laydate({istime: true, format: 'YYYY-MM-DD'})" >
            至
            <input  name="endTime" onfocus="this.blur()" id="endTime" type="text" class="w150" value="<fmt:formatDate value="${endTime }" pattern="yyyy-MM-dd"/>" onclick="laydate({istime: true, format: 'YYYY-MM-DD'})">
          </div>
     <div class="search-item">
       <label>计划名称：</label>
       <input type="text" class="w150" name="planName" value="${planName }">
     </div>
     <div class="search-item">
       <label>计划状态：</label>
     ...
     <div class="btn-content">
       ...
       <a id="tianjia" class="btn btn-xinzeng" href="HealthManagerAction!toAddBPRecoverPlan.action?healthFileId=${healthFileId }">添加</a>
     </div>
   </form>
 </div>
 <!-- 搜索容器 end -->
			...
			<table class="table table-bordered">
				<tbody>
					<tr>
						...
						<td><input type="text" id="start" onfocus="this.blur()"
				...
				<tr>
					...
					<td><input type="text" id="end" onfocus="this.blur()"
						onclick="laydate({istime: true, format: 'YYYY-MM-DD'})"
