...
public class SearchBean {
	...
	private boolean all=false;//用于判定社区集团是否是查全部
	...
	public String getHql(String entityName){
		...
		//判断不同身份责任医生，社区负责人，护士
			if(shenfen==4){
				if(all==true){
					...
					buffer.append(" and entity.communityHospitalId = " + sqid);    //得到该集团下某社区所有慢病老人
				}
			}
			if(shenfen==0){
				buffer.append(" and entity.GId = " + sqid);                    //得到某个集团下的所有慢病老人
			}
			if(shenfen==1 || shenfen == 2){
		...
		if(selectValue != null){
			if(selectValue.equals("") || selectValue == ""){
				...
				if(selectFlag == 0){
					...
					buffer.append(" and entity.fileName like '%"+selectValue+"%' ");
		...
		if(highFileDate != null){//小于最高日期
			...
			buffer.append(" and entity.fileFileDate <= ' "+dateStr+"'");
		}
		...
		if(name != null){
			if(!name.equals("") && name!=""){ //非空的情况
				buffer.append(" and entity.name like '%"+name+"%'");
			}
		}
		//<6>拼入建档人
		if(filePeople !=null){
	...
	public SearchBean(int selectFlag, String selectValue, int highAge,
